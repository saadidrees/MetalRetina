...
def run_finetune(ft_expDate,path_pretrained,ft_fname_data_train_val_test,ft_mdl_name,ft_trainingSamps_dur=-1,
...
# %%
    ...
    #     bias_allCps.append(output_bias.sum())
    #     weights_allCps.append(weights)
    
    # if sum(np.isnan(bias_allCps))>0:
    #     last_cp = np.where(np.isnan(bias_allCps))[0][0]-1     # Where the weights are not nan
    # else:
    #     last_cp = nb_cps-1
    ...
    raw_restored = orbax_checkpointer.restore(fname_latestWeights)
    pre_mdl_state = train_singleretunits.load(mdl,raw_restored['model'],pretrained_params['LR'])
    ...
    ft_path_model_save = os.path.join(path_pretrained,'finetuning',ft_expDate,ft_fname_model)   # the model save directory is the fname_model appened to save path
    ...
    for i in tqdm(range(0,nb_cps_sel+1)): #[nb_cps_sel-1]: #tqdm(range(0,nb_cps_sel-1)):
        ...
        if i==0:    # That is randomly initialized
            ...
            mdl_state_eval = train_metalmaps.load(mdl,raw_restored['model'],ft_lr)
    ...
    idx_best_step = np.argmax(predCorr_medianUnits_allEpochs)

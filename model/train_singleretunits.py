...
from model.models_jax import CNN2D
...
# @jax.jit
# def forward_pass_maps(state,params,batch,training=True):
...
def eval_step(state,data,n_batches=1e5):
    if type(data) is tuple:
        ...
        y_pred = [] #jnp.empty((0,len(state.params['output']['bias'])))
        y = [] #jnp.empty((0,len(state.params['output']['bias'])))
        loss = []
        count_batch = 0
        for batch in data:
...
def train(mdl_state,config,data_train,data_val,batch_size,nb_epochs,path_model_save,save=True,lr_schedule=None,step_start=0,resp_format='INDIVIDUAL',data_info={}):
    ...
    for epoch in tqdm(range(step_start,nb_epochs)):
        ...
        fev_val,_,predCorr_val,_ = model_evaluate_new(y,y_pred,temporal_width_eval,lag=0,obs_noise=0,resp_format=resp_format)

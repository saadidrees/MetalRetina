...
def save_info_unitSplit(fname,info_unitSplit):
    """
    ...
    for key, value in info_unitSplit.items():
        if isinstance(value, np.ndarray):
            if value.dtype == object:  # Handle nested lists
                if all(isinstance(v, np.ndarray) and np.issubdtype(v.dtype, np.number) for v in value):
                    # Case 1: Nested numeric arrays
                    dt = h5py.vlen_dtype(np.dtype('float64'))
                    dset = hf.create_dataset(key, (len(value),), dtype=dt)
                    dset[:] = [np.array(v, dtype=np.float64) for v in value]
                
                elif all(isinstance(v, np.ndarray) and v.dtype.kind in {'U', 'S'} for v in value):
                    # Case 2: Nested string arrays
                    dt = h5py.vlen_dtype(h5py.string_dtype('utf-8'))
                    dset = hf.create_dataset(key, (len(value),), dtype=dt)
                    dset[:] = [np.array(v, dtype=str) for v in value]
                
                else:
                    # Case 3: Mixed string + numbers, convert everything to strings
                    dt = h5py.vlen_dtype(h5py.string_dtype('utf-8'))
                    dset = hf.create_dataset(key, (len(value),), dtype=dt)
                    dset[:] = [np.array([str(item) for item in v], dtype=str) for v in value]

            elif value.dtype.kind in {"U", "S"}:  # String arrays
                dt = h5py.string_dtype(encoding='utf-8')
                hf.create_dataset(key, data=value.astype(dt))
            else:  # Standard numeric arrays
                hf.create_dataset(key, data=value)
                
    f.close()

def load_info_unitSplit(fname):
    """Load an HDF5 file and reconstruct the original dictionary."""
    data = {}

    f = h5py.File(fname, "r")
    with h5py.File(fname, "r") as f:
        grp = f['info_unitSplit']
        for key in grp.keys():
            dataset = grp[key]

            if isinstance(dataset, h5py.Dataset):
                if dataset.dtype.kind in {'i', 'f'}:  # Numeric array
                    data[key] = dataset[()].astype('int')  # Convert to NumPy array
                elif dataset.dtype.kind == 'O':  # Variable-length dataset
                    reconstructed = [np.array(subarr) for subarr in dataset]
                    data[key] = np.array(reconstructed, dtype=object)
                elif dataset.dtype.kind in {'S', 'U'}:  # String array
                    data[key] = np.array(dataset.asstr()[()], dtype=str)
                else:
                    print(f"Skipping key '{key}' due to unknown data type")
            else:
                print(f"Skipping key '{key}' because it's not a dataset")
